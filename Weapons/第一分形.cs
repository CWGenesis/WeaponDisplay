...
namespace WeaponDisplay.Weapons
{
    public class 第一分形 : ModItem
    {
        ...
        private bool GetSparkleGuitarTarget(out List<NPC> validTargets)
        {
            ...
            Rectangle value = Terraria.Utils.CenteredRectangle(player.Center, new Vector2(1000f, 800f));
            for (int i = 0; i < 200; i++)
            {
                NPC npc = Main.npc[i];
        ...
        public override bool Shoot(Player player, EntitySource_ItemUse_WithAmmo source, Vector2 position, Vector2 velocity, int type, int damage, float knockback)
        {
            ...
            for (int num80 = 0; num80 < num79; num80++)
            {
                if (!sparkleGuitarTarget2)
                {
                    ...
                    float num81 = Terraria.Utils.GetLerpValue(0f, 6f, player.velocity.Length(), true) * 0.8f;
                    vector33 *= 1f - num81;
                ...
                float lerpValue2 = Terraria.Utils.GetLerpValue((float)player.itemAnimationMax, 0f, (float)player.itemAnimation, true);
